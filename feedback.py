    ...
    try:
        for item in connection:
            ...
            # ----------------------------------------------------------
                ...
                # ----------------------------------------------------------------------------
                # Send real-time feedback, modifying the flip angle based on repetition number
                # ----------------------------------------------------------------------------
                dataStruct = MyFeedbackData()
                dataStruct.myBool      = True
                dataStruct.myInt64s[0] =  500
                dataStruct.myInt64s[1] = 1000
                dataStruct.myFloat     =    1.23

                # Scale the flip angle by the repetition number
                dataStruct.myFloat     =    1.0-((item.getHead().repetition+2.0)/(metadata.encoding[0].encodingLimits.repetition.maximum+1.0))
                logging.info("Received repetition %d of %d -- requesting flip angle scaling of next image by %1.2f%%" % (item.getHead().repetition, metadata.encoding[0].encodingLimits.repetition.maximum, dataStruct.myFloat))
                connection.send_feedback("MyFeedback", dataStruct)

                logging.debug("Created and sent feedback struct with data:")
                logging.debug("dataStruct.myBool    is: %s" %      dataStruct.myBool)
                logging.debug("dataStruct.myInt64s are: %s" % list(dataStruct.myInt64s))
                logging.debug("dataStruct.myFloats  is: %s" %      dataStruct.myFloat)

                # Show the underlying data being sent
                buf = (ctypes.c_uint8 * ctypes.sizeof(dataStruct))()
                ctypes.memmove(buf, ctypes.byref(dataStruct), ctypes.sizeof(dataStruct))
                logging.debug("dataStruct is %s bytes, serialized as: %s" % (ctypes.sizeof(dataStruct), list(buf)))

                # Only process magnitude images -- send phase images back without modification (fallback for images with unknown type)
                if (item.image_type is ismrmrd.IMTYPE_MAGNITUDE) or (item.image_type == 0):
                    # Don't do the "invert contrast" -- this example does not return images, only feedback
                    pass
